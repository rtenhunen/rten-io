...
class LanguagePack::Ruby < LanguagePack::Base
  ...
  RBX_BASE_URL         = "https://af-ops.siri.apple.com/artifactory/orchard-languages/rbx"
  ...
  def build_bundler
    instrument 'ruby.build_bundler' do
      log("bundle") do
        ...
        # add bundle config from env var
        log("bundle", "config") do
          if ENV.key? 'BUNDLE_BUILD_CONFIG'
            puts "Environment variable $BUNDLE_BUILD_CONFIG is set."
            ENV['BUNDLE_BUILD_CONFIG'].split(';').each do |bundle_build_config|
              unless bundle_build_config.nil? or bundle_build_config.empty?
                bundle_config_cmd = "#{bundle_bin} config --local #{bundle_build_config}"
                puts "running command: '#{bundle_config_cmd}'"
                run_stdout(bundle_config_cmd)
              end
            end
          end
        end
